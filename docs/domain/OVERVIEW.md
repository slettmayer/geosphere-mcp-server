...
conditions and forecasts at any coordinate **inside Austria and the Alpine region**, from high-resolution
GeoSphere Austria data. Coordinates outside that region are answered with an out-of-coverage line rather
than served from a second source. It replaces an OpenWeatherMap MCP server and keeps that server's tool
names so agent routing prompts transfer unchanged.
...
|---------|----------|--------------------|--------|
| Four MCP tools (current, hourly, storm outlook, air quality) | The user's AI assistant | MCP over stdio; no auth, launched locally by the client | [OUTPUT-CONTRACT.md](OUTPUT-CONTRACT.md) |
...
|---------|-----------|----------|
| Data sources | The GeoSphere dataset catalog | [DATA-SOURCES-AND-COVERAGE.md](DATA-SOURCES-AND-COVERAGE.md) |
| Coverage | Dynamic coverage detection and the three coverage classes | [DATA-SOURCES-AND-COVERAGE.md](DATA-SOURCES-AND-COVERAGE.md) |
...
| Condition vocabulary | The Home Assistant condition set the server emits | [CONDITION-DERIVATION.md](CONDITION-DERIVATION.md) |
| Condition derivation | Derivation from the physical parameters, not a symbol code | [CONDITION-DERIVATION.md](CONDITION-DERIVATION.md) |
...
| **out-of-domain** | A GeoSphere 400 carrying "outside of dataset bounds" — the coverage signal, rendered as a distinct out-of-coverage line |
| **rate limit** | A GeoSphere 429 — retried once when the wait is 5 s or less, else returned as a notice; unlike out-of-domain it is worth retrying |
...
- **Dynamic coverage detection**: attempt the fetch and let the API's own out-of-domain answer decide — no
  bounding box to maintain as the grids evolve.
- **One source, no worldwide fallback**: an Open-Meteo fallback existed through v0.3.x and was removed —
  see the design decision in [DATA-SOURCES-AND-COVERAGE.md](DATA-SOURCES-AND-COVERAGE.md).
- **Markdown output in metric units**, compact and friendly to assistants that read results aloud.
- **Degrade rather than fail**: rate limits and secondary-dataset failures have a defined partial-success
  path, and an uncovered point gets a plain refusal rather than a misleading error.

## Known Risks
- GeoSphere resource IDs are versioned — a catalog rotation breaks the primary path until the IDs in
  `const.py` are bumped.
- No GeoSphere forecast reaches beyond ~60 h, and with no fallback source there is no multi-day answer
  this server can give at all.
- AROME publishes convective inhibition as a negative magnitude and the thunder gate assumes that sign
  without checking; a source that publishes it positive would silently invert the gate.
