...
`__version__` literal, no `version =` in `pyproject.toml`. `src/geosphere_mcp_server/_version.py` is generated
...
0.1.1, 0.1.2 and 0.2.1 were published with no section. Step 2 above is what closes that.
